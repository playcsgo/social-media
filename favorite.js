        ...
        <div class="card mb-1">
          ...
          <div class="card-body">
            ...
            <footer>
              ...
              id}">More</a>
              <a href="#" class="btn btn-danger btn-remove" data-id="${user.
                id}">x</a>
            </footer>
          </div>
        </div>
...
const controller = {
  ...
  addSearchEventLisenter () {
    searchForm.addEventListener('submit', function onSearchFrom(event) {
      ...
      model.filteredUsers = model.favoriteUsers.filter(user => 
  ...
  addUserCardListener() {
    userList.addEventListener('click', function userListClicked(event) {
      ...
      const user = model.favoriteUsers.find(user => user.id === id)
      ...
      if (event.target.classList.contains('btn-remove')) {
        controller.removeFromFavorite(user)
        view.renderUserList(controller.getUsersByPage(view.curPage))
        view.renderPaginator(model.favoriteUsers)
      }
    })
  },
  showModalInfo(user) {
  ...
  getUsersByPage(page) {
    const data = model.filteredUsers.length ? model.filteredUsers : model.favoriteUsers
  ...
  removeFromFavorite(user) {
    model.favoriteUsers = JSON.parse(localStorage.getItem('favoriteUsers'))
    const index = model.favoriteUsers.findIndex(favoriteUser => favoriteUser.id === user.id)
    model.favoriteUsers.splice(index, 1)
    localStorage.setItem('favoriteUsers', JSON.stringify(model.favoriteUsers))
  }
}
